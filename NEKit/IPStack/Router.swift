        ...
        if packet.sourceAddress == interfaceIP {
            if packet.sourcePort == proxyServerPort {
                guard let (address, port) = IPv4NATRoutes[packet.destinationPort] else {
                    nelog("Does not know how to handle packet: \(packet) because can't find entry in NAT table.")
                    return nil
                }
            ...
            nelog("Does not know how to handle packet.")
            return nil
        }
    }
    ...
    func readAndProcessPackets() {
        NetworkInterface.TunnelProvider.packetFlow.readPacketsWithCompletionHandler() {
            ...
            for packet in packets {
                nelog("Received packet of type: \(packet.proto) from \(packet.sourceAddress) to \(packet.destinationAddress)")
                if let packet = self.rewritePacket(packet) {
                    outputPackets.append(packet)
                } else {
                    nelog("Failed to rewrite packet \(packet)")
                }
            }
            ...
            if outputData.count > 0 {
                nelog("Write out \(outputData.count) packets.")
